...
pub async fn run_optimized_batch(pool: DbPool) -> Result<()> {
    ...
    for (did, handle) in &followers_map {
        process_user(did, Some(handle), &pool, &keypair, &conf.labeler_did).await?;
        tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    }

    for did in local_dids {
        if !followers_map.contains_key(&did) {
            negate_user(&did, &pool, &keypair, &conf.labeler_did).await?;
            tokio::time::sleep(std::time::Duration::from_millis(50)).await;
        }
    }
